...
/* ------------------------------------------------------------------ */
function bindToToolResult(_input) {
    // Needs conversation context we don't have — pass-through for now.
    return { pass: true };
}
function assumptionsLabeled(input) {
...
function actionAdvancesReason(input) {
    ...
    const actionTokens = tokenize(`${input.tool} ${safeStringify(input.args)}`);
    let overlap = 0;
    ...
    if (overlap === 0) {
        return {
            pass: false,
            reason: "Zero keyword overlap between stated reason and tool/args — action may not advance the given reason.",
        };
    }
    return { pass: true };
}
function noExfiltrationDetected(input) {
    const text = argsString(input.args);
...
function authorizedTarget(input) {
    const text = argsString(input.args);
...
function noManipulationDetected(input) {
    const text = argsString(input.args);
